...
start = current_dateTime-dt.timedelta(days=100)

st.set_page_config(page_title="Swing", page_icon="🔎")

st.title("Swing Trading Breakouts")

ticker=st.text_input('Ticker','AAPL,DOCU')
ticker=ticker.split(",")

number = st.number_input("Insert a number of candles", value=24, placeholder="Type a number...")
periodos_atr = st.number_input("Insert a lenght for ATR", value=6, placeholder="Type a number...")
...
for i in ticker:
    data = yf.download(i,interval = '1h', start=start, end=current_dateTime)
...
dcc_atr = {}
for x in ticker:
    data_atr = yf.download(x, interval='1mo')
